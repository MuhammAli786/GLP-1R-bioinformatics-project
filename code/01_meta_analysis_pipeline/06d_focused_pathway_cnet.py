...
def build(pathway, thr):
    ...
    ax.set_title(f"{TITLES[pathway]} Cnet - focused enrichment  ({THR_FOLDER[thr]})",
                 fontsize=24, fontweight="bold", pad=20); ax.axis("off")
    for fmt in ("PNG", "PDF"):
        d = os.path.join(PLOTS, pathway, THR_FOLDER[thr], fmt); os.makedirs(d, exist_ok=True)
        fig.savefig(os.path.join(d, f"Cnet_{pathway}_FocusedTerms_{THR_FOLDER[thr]}.{fmt.lower()}"),
                    dpi=DPI, bbox_inches="tight", transparent=True)
    plt.close(fig)


if __name__ == "__main__":
    pathway = sys.argv[1] if len(sys.argv) > 1 else "Survival"
    thr = sys.argv[2] if len(sys.argv) > 2 else "LFC02"
    build(pathway, thr)
    print("done")
